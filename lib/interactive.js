...
function debounce(fn, ms) {
  ...
  return (...args) =>
    new Promise((resolve, reject) => {
      clearTimeout(timer);
      timer = setTimeout(() => fn(...args).then(resolve).catch(reject), ms);
    });
}
...
const searchNpm = debounce(async (input) => {
  ...
  try {
    ...
    return [{ name: '(npm unreachable — check your connection)', value: null, disabled: true }];
  }
  if (!res.ok) return [];
...
async function addManually(templates, name) {
  ...
  const pin = await p.select({
    ...
    options: [
      { value: true,  label: 'Pin to latest', hint: 'e.g. axios@1.7.9 — reproducible' },
      { value: false, label: 'Latest only',   hint: 'e.g. axios — always installs newest' },
    ],
  });
      ...
      pkg = await search({
        ...
        source: async (input) => {
          ...
          return objects.map(({ package: pkg }) => ({
            ...
            disabled: [...existing].some((e) => baseName(e) === pkg.name) ? 'already added' : false,
          }));
        },
      });
    } catch {
...
async function addFromProject(templates, name) {
  ...
  const options = [
    ...deps.map(([n, v])    => ({ value: `dep:${n}@${v}`,    label: n, hint: `dependency  ${v}` })),
    ...devDeps.map(([n, v]) => ({ value: `devDep:${n}@${v}`, label: n, hint: `devDependency  ${v}` })),
  ];
  ...
  const selectedDeps    = selected.filter((v) => v.startsWith('dep:')).map((v) => v.slice(4));
  const selectedDevDeps = selected.filter((v) => v.startsWith('devDep:')).map((v) => v.slice(7));
...
async function editManagePackages(templates, name) {
  ...
  const options = [
    ...dependencies.map((d)    => ({ value: `dep:${d}`,    label: baseName(d), hint: `dependency  ${d.includes('@') ? d.split('@').pop() : ''}` })),
    ...devDependencies.map((d) => ({ value: `devDep:${d}`, label: baseName(d), hint: `devDependency  ${d.includes('@') ? d.split('@').pop() : ''}` })),
  ];
  ...
  const keptDeps    = selected.filter((v) => v.startsWith('dep:')).map((v) => v.slice(4));
  const keptDevDeps = selected.filter((v) => v.startsWith('devDep:')).map((v) => v.slice(7));
...
function npmInstallAsync(flag, pkg) {
  return new Promise((resolve, reject) => {
    const child = spawn('npm', ['install', flag, pkg], { stdio: 'pipe', shell: true });
    let stderr = '';
...
async function handleApply() {
  ...
  if (dependencies.length)    totalFailed += await installWithProgress(dependencies, false);
  if (devDependencies.length) totalFailed += await installWithProgress(devDependencies, true);

  const succeeded = total - totalFailed;
  if (totalFailed === 0) {
    p.log.success(`Template "${name}" applied — ${succeeded}/${total} packages installed.`);
  } else {
    p.log.warn(`Template "${name}" applied with issues — ${succeeded}/${total} installed, ${totalFailed} failed.`);
  }
}
...
async function handleExport() {
  ...
  const defaultName = selected.length === 1 ? `${selected[0]}.json` : 'pakio-export.json';
...
async function handleImportFile() {
  const filepath = await p.text({
    ...
    validate: (v) => {
      ...
      if (!fs.existsSync(path.resolve(process.cwd(), v.trim()))) return 'File not found.';
    },
  });
  ...
  let incoming;
  try {
    const raw = fs.readFileSync(path.resolve(process.cwd(), filepath.trim()), 'utf8');
    incoming = JSON.parse(raw);
  } catch {
    p.log.error('Could not read or parse the file. Make sure it is a valid pakio JSON export.');
    return;
  }
  ...
  const selected = await p.multiselect({
    ...
    options: incomingNames.map((n) => ({
      ...
      label: n,
      hint: [
        incoming[n].dependencies?.length    && `${incoming[n].dependencies.length} dep`,
        incoming[n].devDependencies?.length && `${incoming[n].devDependencies.length} devDep`,
      ].filter(Boolean).join(', '),
    })),
